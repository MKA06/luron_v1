                                ...
                                async for message in elevenlabs_ws:
                                    ...
                                    if data.get('audio'):
                                        ...
                                        if stream_sid and not cancel_ai_response.is_set():
                                            ...
                                            # Add small pacing delay to prevent overwhelming Twilio's buffer
                                            # For ulaw_8000: 8000 bytes = 1 second of audio
                                            # Calculate duration and add minimal delay for smooth delivery
                                            audio_bytes = len(base64.b64decode(audio_data))
                                            audio_duration_ms = (audio_bytes / 8000.0) * 1000
                                            # Use 70% of actual duration to stay ahead but maintain smooth flow
                                            pacing_delay = (audio_duration_ms * 0.7) / 1000.0
                                            # Minimum delay to prevent flooding, maximum to prevent lag
